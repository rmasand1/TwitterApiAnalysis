...
consumer_key='nJv4RriazD1OYZo13TpRAZigZ'
consumer_secret= 'DiJHOPGONsoHCkdM2MyGVxrYL0gNpwIdvcGkXJqIsdyJ1sJXk8'
access_token= '760059806-NPO4XCmgi0zOJNpktEYjQ2nkr4kV9dQyix44KKld'
access_token_secret= 'KDpbCQHywCGRyRDTVmA9hxebCpyRAyDMAh65TytFIem3X'
...
def friend_overlap(users):
    ...
    for i in range(0,len(users)):
        u_set.append(users[i]['friends'])
    
    u_set1 = [set(u_set[i]).intersection(u_set[j]) for i in range(0,len(users)) for j in range(i+1,len(users))]
    c=0
    overlapped=[]
    
    for i in range(0,len(users)):
        for j in range(i+1,len(users)):
            overlapped.append((users[i]['screen_name'],users[j]['screen_name'],len(u_set1[c])))
            c+=1
    return sorted(overlapped, key=lambda x:x[2], reverse=True)
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    clinton=users[2]['friends']
    trump=users[3]['friends']
    clintontrump=set(clinton).intersection(trump)
    common=list(clintontrump)
    request=robust_request(twitter,'users/lookup', {'user_id':common[0]})
    users = [i for i in request]
    return users[0]['name']
...
def create_graph(users, friend_counts):
    ...
    for i in range(0,len(users)):
        for x in users[i]['friends']:
            if friend_counts[x]>1:
                graph.add_edge(users[i]['screen_name'],x)
    return graph
...
def draw_network(graph, users, filename):
    ...
    """
    ###TODO
    gary=users[0]['screen_name']
    jill=users[1]['screen_name']
    clinton=users[2]['screen_name']
    trump=users[3]['screen_name']
    labels={gary:users[0]['screen_name'], jill:users[1]['screen_name'],clinton:users[2]['screen_name'],trump:users[3]['screen_name']}
    plt.figure(figsize=(20,20))
    nx.draw_networkx(graph,labels=labels,width=0.3,node_size=95)
    plt.axis('off')
